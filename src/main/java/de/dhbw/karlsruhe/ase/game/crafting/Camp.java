...
public class Camp {
    ...
    public void devastate() {
        ...
        for (final Buildable b : all) { //TODO: maybe devastate more here
     ...
     * @throws IllegalGameInstructionException if the Resources are not sufficient, the buildable already exists,
     *                                         or other criteria for constructing the Buildable
     *                                         (such as an existing fireplace) are not met
     */
    public Buildable build(final CraftingPlan plan) throws IllegalGameInstructionException {
        for (final Buildable b : constructed)
            if (b.getCraftingPlan() == plan)
                throw new IllegalGameInstructionException("buildable already exists. Building the same thing "
